...
describe Tokens::Ident do
  it 'cannot be empty' do
    expect('').not_to parse_to(Tokens::Ident)
  end

  it 'can be composed of a single alphabetic character' do
    ['a', 'T', 'é', 'λ', 'Ж'].each do |s|
      expect(s).to parse_to(Tokens::Ident)
    end
  end

  it 'can be composed of a single underscore' do
    expect('_').to parse_to(Tokens::Ident)
  end

  it 'cannot be composed of a digit or punctuation' do
    ['0', '9', '-', '!', '?'].each do |s|
      expect(s).not_to parse_to(Tokens::Ident)
    end
  end
  ...
  it 'cannot have a embedded bang' do
    expect('a!a').not_to parse_to(Tokens::Ident)
  end
